...
class MP_Exporter_Orders {
	...
	// Date filtering with proper sanitization and validation
	if ( isset($_POST['m']) && $_POST['m'] > 0 ) {
		...
		if ($year >= 1970 && $year <= 2100) {
			...
			if ( strlen($date_filter) > 7 ) {
				$day = (int) substr($date_filter, 6, 2);
				if ($day >= 1 && $day <= 31) {
					$query .= $wpdb->prepare(" AND DAYOFMONTH($wpdb->posts.post_date) = %d", $day);
				}
			}
			
			if ( strlen($date_filter) > 9 ) {
				$hour = (int) substr($date_filter, 8, 2);
				if ($hour >= 0 && $hour <= 23) {
					$query .= $wpdb->prepare(" AND HOUR($wpdb->posts.post_date) = %d", $hour);
				}
			}
			
			if ( strlen($date_filter) > 11 ) {
				$minute = (int) substr($date_filter, 10, 2);
				if ($minute >= 0 && $minute <= 59) {
					$query .= $wpdb->prepare(" AND MINUTE($wpdb->posts.post_date) = %d", $minute);
				}
			}
			
			if ( strlen($date_filter) > 13 ) {
				$second = (int) substr($date_filter, 12, 2);
				if ($second >= 0 && $second <= 59) {
					$query .= $wpdb->prepare(" AND SECOND($wpdb->posts.post_date) = %d", $second);
				}
			}
		}
	...
	if ( isset($_POST['m']) ) {
		// Sanitize for filename: only alphanumeric characters
		$date_suffix = preg_replace('/[^0-9]/', '', $_POST['m']);
		$filename .= '_' . sanitize_file_name($date_suffix);
	}
	$filename .= '_' . time() . '.csv';		//serve the file
		rewind($file);
