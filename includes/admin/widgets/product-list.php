...
class MarketPress_Product_List extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array( 'classname' => 'mp_widget mp_widget_products_list', 'description' => __( 'Zeigt dir eine anpassbare Produktliste aus deinem MarketPress-Shop.', 'mp' ) );
		parent::__construct( 'mp_product_list_widget', __( 'Produktliste', 'mp' ), $widget_ops );
	}
	...
	function form( $instance ) {
		...
		?>
		<p><label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Titel:', 'mp' ) ?> <input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" /></label></p>
		<p><label for="<?php echo $this->get_field_id( 'custom_text' ); ?>"><?php _e( 'Eigener Text:', 'mp' ) ?><br />
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'num_products' ); ?>"><?php _e( 'Anzahl Produkte:', 'mp' ) ?> <input id="<?php echo $this->get_field_id( 'num_products' ); ?>" name="<?php echo $this->get_field_name( 'num_products' ); ?>" type="text" size="3" value="<?php echo $num_products; ?>" /></label><br />
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'order_by' ); ?>"><?php _e( 'Produkte sortieren nach:', 'mp' ) ?></label><br />
			<select id="<?php echo $this->get_field_id( 'order_by' ); ?>" name="<?php echo $this->get_field_name( 'order_by' ); ?>">
				...
				<option value="date"<?php selected( $order_by, 'date' ) ?>><?php _e( 'Veroeffentlichungsdatum', 'mp' ) ?></option>
				...
				<option value="sales"<?php selected( $order_by, 'sales' ) ?>><?php _e( 'Anzahl Verkaeufe', 'mp' ) ?></option>
				<option value="price"<?php selected( $order_by, 'price' ) ?>><?php _e( 'Produktpreis', 'mp' ) ?></option>
				<option value="featured"<?php selected( $order_by, 'featured' ) ?>><?php _e( 'Hervorgehoben', 'mp' ) ?></option>
				<option value="rand"<?php selected( $order_by, 'rand' ) ?>><?php _e( 'Zufaellig', 'mp' ) ?></option>
			</select><br />
		...
		<p>
			...
			<input id="<?php echo $this->get_field_id( 'taxonomy' ); ?>" name="<?php echo $this->get_field_name( 'taxonomy' ); ?>" type="text" size="17" value="<?php echo $taxonomy; ?>" title="<?php _e( 'Slug eingeben', 'mp' ); ?>" />
		</p>
		...
		<p><input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id( 'show_only_featured' ); ?>" name="<?php echo $this->get_field_name( 'show_only_featured' ); ?>"<?php checked( $show_only_featured ); ?> />
			<label for="<?php echo $this->get_field_id( 'show_only_featured' ); ?>"><?php _e( 'Nur hervorgehobene Produkte anzeigen', 'mp' ); ?></label><br />
			...
			<label for="<?php echo $this->get_field_id( 'show_thumbnail' ); ?>"><?php _e( 'Vorschaubild anzeigen', 'mp' ); ?></label><br />
			<label for="<?php echo $this->get_field_id( 'size' ); ?>"><?php _e( 'Vorschaubild-Groesse:', 'mp' ) ?> <input id="<?php echo $this->get_field_id( 'size' ); ?>" name="<?php echo $this->get_field_name( 'size' ); ?>" type="text" size="3" value="<?php echo $size; ?>" /></label></p>
		...
		<p><input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id( 'show_excerpt' ); ?>" name="<?php echo $this->get_field_name( 'show_excerpt' ); ?>"<?php checked( $show_excerpt ); ?> />
			<label for="<?php echo $this->get_field_id( 'show_excerpt' ); ?>"><?php _e( 'Kurzbeschreibung anzeigen', 'mp' ); ?></label><br />
			...
			<label for="<?php echo $this->get_field_id( 'show_thumbnail_placeholder' ); ?>"><?php _e( 'Vorschaubild-Platzhalter anzeigen (wenn kein Bild gesetzt ist)', 'mp' ); ?></label><br />
			...
			<label for="<?php echo $this->get_field_id( 'show_button' ); ?>"><?php _e( 'Kaufen-Button anzeigen', 'mp' ); ?></label></p>
