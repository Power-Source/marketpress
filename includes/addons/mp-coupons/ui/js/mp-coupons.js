( function( $ ) {
	...
	$.mptooltip.prototype = {
		...
		open: function() {
			var self = this;
			if ( !this.tooltip ) {
				this.tooltip = $( '<div class="mp-tooltip ' + this.options.tooltipClass + '">' + this.options.content + '</div>' )
					.insertAfter( this.$element );
			} else {
				this.tooltip.removeClass( 'error success' ).addClass( this.options.tooltipClass );
				this.tooltip.text( this.options.content );
			}
			this.tooltip.fadeIn( this.options.show );
			this.$element.trigger( 'tooltipopen.mp_coupons' );
		},
		close: function() {
			if ( this.tooltip ) {
				this.tooltip.fadeOut( this.options.hide, function() {
					$( this ).remove();
				} );
				this.tooltip = null;
			}
	...
	$.fn.mptooltip = function( methodOrOptions ) {
		if ( typeof methodOrOptions === 'string' ) {
			...
			return this.each( function() {
				var instance = $( this ).data( 'mptooltip' );
				if ( !instance ) {
					instance = new $.mptooltip( this );
					$( this ).data( 'mptooltip', instance );
				}
			...
			// Init call
			return this.each( function() {
				var instance = new $.mptooltip( this, methodOrOptions );
				$( this ).data( 'mptooltip', instance );
			} );
		}
	};
