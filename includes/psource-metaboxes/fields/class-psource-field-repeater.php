...
jQuery(document).ready(function($){
	...
	var initializeDragAndDrop = function() {
		...
		$('.psource-subfields').not('.no-sorting').on('dragstart', '.psource-subfield-group', function(e) {
			draggedElement = this;
			$(this).css('opacity', '0.5');
			e.originalEvent.dataTransfer.effectAllowed = 'move';
		}).on('dragend', '.psource-subfield-group', function(e) {
			draggedElement = null;
			$(this).css('opacity', '1');
		}).on('dragover', '.psource-subfield-group', function(e) {
			if (draggedElement && draggedElement !== this) {
				e.preventDefault();
				if ($(this).offset().top < $(draggedElement).offset().top) {
					$(draggedElement).insertBefore(this);
				} else {
					$(draggedElement).insertAfter(this);
				}
				updateOrdering($(this).closest('.psource-subfields').find('.psource-subfield-group'));
			}
		}).on('drop', '.psource-subfield-group', function(e) {
			e.preventDefault();
			if (draggedElement) {
				$(document).trigger('psource_repeater_field/stop_sort', [ $(draggedElement) ]);
			}
		});
		
		// Make group headers draggable
		$('.psource-subfield-group-index').css('cursor', 'move');
		$('.psource-subfield-group').attr('draggable', 'true');
	};
	...
	$('.psource-repeater-field-add').on('click', function(){
		...
		var currentCount = $subfields.find('.psource-subfield-group').length;
		$clonedRow.find('.psource-subfield-group-index span').text(currentCount);
		...
		updateOrdering($subfields.find('.psource-subfield-group'));
